...
export const CartProvider = ({children}) => {
    ...
    const addItemToCart = (product) => {
        const inCart = cartItems.find((productInCart) => productInCart.id_product === product.id_product
        ); 
    ...
    const deleteItemToCart = (product) => {
            ...
            if(inCart.amount === 1){
                setCartItems(
                    cartItems.filter((productInCart) => productInCart.id_product !== product.id_product)
                );
            } else {
                setCartItems(
                    cartItems.map((productInCart) => {
